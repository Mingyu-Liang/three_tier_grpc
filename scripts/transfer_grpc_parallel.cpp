...
for i in services_json[serviceName]['rpcs_send']:
  ...
  cog.outl('CommonCallData* call):AbstractAsyncClientCall(), call_(call) {')
  ...
  cog.outl('')
  cog.outl('void Forward(CommonCallData* call) {')
  cog.outl('Async' + rpc_name + '_ClientCall* call_ = new Async' + rpc_name + '_ClientCall(cq_, stub_, call);}')
  cog.outl('')
...
for i in services_json[serviceName]['rpcs_receive']:
  ...
  cog.outl('int ExpectedResponseNumber;')
  ...
  cog.outl('virtual void Proceed(bool = true) override {')
    ...
    if len(rpcs_send_dep[rpc_send]) == 0:
      ...
      for j in range(len(rpcs_send_dep[rpc_send])):
        if j == len(rpcs_send_dep[rpc_send]) - 1:
          cog.outl('sent[' + str(rpcs_send_list.index(rpcs_send_dep[rpc_send][j])) + '] == true) {')
        else:
          cog.outl('sent[' + str(rpcs_send_list.index(rpcs_send_dep[rpc_send][j])) + '] == true &&')
      if rpc_send in rpc_replica_map:
        cog.outl('rpc_' + str(rpc_replica_map[rpc_send]) + '_client_->Forward(this);')
  ...
  ExpectedResponseNumber = 0
  for j in range(len(rpcs_receive_send[i])):
    ExpectedResponseNumber += rpcs_receive_send[i][j][1]
    for k in range(rpcs_receive_send[i][j][1]):
      cog.outl('rpc_' + str(rpcs_receive_send[i][j][0]) + '_client_->Forward(this);')
  cog.outl('')
  cog.outl('ExpectedResponseNumber = ' + str(ExpectedResponseNumber) + ';')
  cog.outl('')
  ...
  if len(rpcs_receive_send[i]) > 0:
    cog.outl('if (--ExpectedResponseNumber == 0) {')
  cog.outl('span->Finish();')
  cog.outl('responder_.Finish(reply_, Status::OK, this);')
  cog.outl('status_ = DESTROY; }')
  if len(rpcs_receive_send[i]) > 0:
