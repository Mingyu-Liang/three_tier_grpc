...
class Rpc_2_CallData : public CommonCallData {
 public:
  ...
  Rpc_2_CallData(service_2::AsyncService* service, ServerCompletionQueue* cq,
      ...
      : CommonCallData(service, cq), rpc_2_proc_dist_(rpc_2_proc_dist) {
    Proceed();
  }
  ...
  virtual void Proceed(int ID = -1) override {
    if (status_ == CREATE) {
      ...
      new Rpc_2_CallData(service_, cq_, rpc_2_proc_dist_);
...
class ServerImpl final {
 public:
  ...
  void Run() {
    ...
    new Rpc_2_CallData(&service_, cq_.get(), &rpc_2_proc_dist);
